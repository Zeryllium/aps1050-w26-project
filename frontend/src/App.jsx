...
import TokenArtifact from "./contracts/LightToken.json";
...
import "./index.css";
import {textH1Style} from "./ui/ui.js";
...
export class Dapp extends React.Component {
  ...
  requestFunds = async() => {
    try {
      ...
      const tx = await this.state._wagerFaucet.requestFromFaucet(userAddress);
      await tx.wait();
  ...
  render() {
    ...
    return (
        <div className="container p-4">
          ...
          <button className={`${UI.buttonGrey}`} onClick={this.requestFunds}>Faucet</button>
  ...
  async _initializeEthers() {
    ...
    const faucetInstance = new ethers.Contract(
    ...
    const wagerInstance = new ethers.Contract(
        contractAddress.Wager,
        WagerArtifact.abi,
        this._provider.getSigner(0)
    );
    ...
    this.setState({
      ...
      _oracle: oracleInstance,
      _wagerFaucet: faucetInstance,
      _wager: wagerInstance
    }, () => {
      this._getTokenData();
  ...
  async _getTokenData() {
    const name = await this.state._token.name();
    const symbol = await this.state._token.symbol();

    this.setState({ tokenData: { name, symbol } });
  }

  async _updateBalance() {
    const rawBalance = await this.state._token.balanceOf(this.state.selectedAddress);
  ...
  async _transferTokens(to, amount) {
    ...
    try {
      ...
      const tx = await this.state._token.transfer(to, amount);
